<template>
  <div class="flex-row-between">
    <h1>收费标准</h1>
    <el-button type="primary" @click="isShowDialog = true"> 修改 </el-button>
  </div>
  <!-- 显示收费标准 -->
  <h2 class="flex-row" v-for="(item, index) in chargeRates" :key="index">
    <img class="w-48px" src="@/assets/img/img-tea.png" alt="·" />
    <template v-if="item.type == 0"> {{ item.district[0] }} 到 {{ item.district[1] }} 小时内，一共收费 {{ item.price }} 元 </template>
    <template v-if="item.type == 1"> {{ item.district[0] }} 到 {{ item.district[1] }} 小时内，每小时收费 {{ item.price }} 元 </template>
  </h2>
  <!-- 修改收费标准 -->
  <el-dialog v-model="isShowDialog" title="修改收费标准" width="980" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-button type="primary" @click="addCharge"> + 新增时间阶段 </el-button>
    <div class="flex-row my-2" v-for="(item, index) in temporaryCharge" :key="index">
      ...
      <el-select class="ml-4 mr-2 max-w-40" v-model="item.type" placeholder="收费方式">
        <el-option label="区间收费" :value="0" />
        <el-option label="按时收费/小时" :value="1" />
      </el-select>
      <span>金额：</span>
      <el-input-number v-model="item.price" :min="0" />
      <el-button class="ml-2" type="danger" @click="deleteCharge(index)"> 删除 </el-button>
    </div>
    <template #footer>
