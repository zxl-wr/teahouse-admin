<template>
  ...
  <el-dialog style="width: fit-content" v-model="isShowTransfer" :title="getTableName(currentOrder.table_id)" top="0" v-if="currentOrder">
    <div class="text-6 text-right c-#f56c6c">￥{{ getCurrentOrderPrice(currentOrder).toFixed(2) }}</div>
    <div class="flex">
      <!-- 商品列表 -->
      <div class="w-300px h-400px overflow-auto">
        ...
        <div class="flex-row-between card m-1 p-2 select-none hover:bg-#2cbf9c hover:c-#fff" v-for="goods in currentGoodsList" :key="goods.id" @dblclick="addOrderGoods(goods)">
          <div>{{ goods.name }}</div>
          <div>￥{{ goods.price }}</div>
        </div>
      </div>
      <!-- 已购商品 -->
      <el-table class="!w-500px !h-400px" :data="currentOrder.goods_list" border>
  ...
  <el-dialog style="width: fit-content" v-model="isShowBill" title="结账" top="0">
    ...
    <template #footer>
      <el-button type="danger" v-print="printObj">结账打印</el-button>
    </template>
  </el-dialog>
</template>
